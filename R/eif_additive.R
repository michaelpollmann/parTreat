...
#' @param ... additional arguments passed on estimate_density_d_logs for density estimation. DO NOT DISABLE ESTIMATION OF DERIVATIVES.
#' @return list of two elements if calc_se = TRUE or one element otherwise: \item{tau}{the point estimate of the treatment effect} \item{se}{the estimated standard error (if calc_se = TRUE)}
#' @examples
...
eif_additive <- function(X, Y, xf=NULL, d1_logf=NULL, d2_logf=NULL, theta_init=NULL,calc_se=TRUE,...) {
    ...
    if (is.null(d1_logf)) {
        dens <- estimate_density_d_logs(X=X,...)
        xf <- dens$xf
